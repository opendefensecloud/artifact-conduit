...
type ClusterEndpointLister interface {
	...
	List(selector labels.Selector) (ret []*arcv1alpha1.ClusterEndpoint, err error)
	// Get retrieves the ClusterEndpoint from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*arcv1alpha1.ClusterEndpoint, error)
...
type clusterEndpointLister struct {
	listers.ResourceIndexer[*arcv1alpha1.ClusterEndpoint]
}
...
func NewClusterEndpointLister(indexer cache.Indexer) ClusterEndpointLister {
	return &clusterEndpointLister{listers.New[*arcv1alpha1.ClusterEndpoint](indexer, arcv1alpha1.Resource("clusterendpoint"))}
}
