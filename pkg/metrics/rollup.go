...
//
// Precedence is deliberate: Failed, then Running, then Succeeded, then Pending.
// A failure anywhere makes the whole Order failed. In flight work outranks
// completed work, so an Order reads Succeeded only once every workflow has
// succeeded.
//
// WorkflowStopped is unreachable: Argo's WorkflowPhase has no Stopped, so ARC
// never assigns it. Give it a branch here if that changes.
//
